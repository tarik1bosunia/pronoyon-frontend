...
const SCHOOL_YEARS = ['২০২৩', '২০২২', '২০২১', '২০২০', '২০১৯', '২০১৮'];
...
export function SchoolYearSelectModal({ isOpen, onClose, selectedYears, onYearsChange }: Props) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>স্কুল/কলেজ বছর নির্বাচন করুন</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3 max-h-80 overflow-y-auto">
            {SCHOOL_YEARS.map(year => (
              <div key={year} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded">
                <Checkbox 
                  id={`modal-school-year-${year}`}
                  checked={selectedYears.includes(year)}
                  onCheckedChange={() => handleToggle(year)}
                />
                <label 
                  htmlFor={`modal-school-year-${year}`}
